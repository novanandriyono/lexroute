...
class RouteUnset extends Command
{
     ...
     * @var string
     */
    protected $signature = 'route:unset {name?} {--a|api} {--c|cache} {--i|info}';
    ...
    protected function getRouteOption($route = null){
        $route = key($route);
        if(str_contains($route,"middleware") === true){
            $results[] = "middleware";
        }
        if(isset($results) === false){
            $this->info('noting to unset');
            return exit();
        }
        return $results;
    }
